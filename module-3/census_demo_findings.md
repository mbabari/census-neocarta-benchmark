...
## Be honest about what this demo does NOT show

- It is **not** a clean "Neocarta always uses fewer tokens" story.
- A **strong** model (`gpt-4o`) is good enough to guess each table from its name
  and finds it in ~3 cheap calls, so on these questions Neocarta's retrieval
  (which returns the full ~252-column table) actually costs **more**. Token/cost
  reduction is a **weak-model** effect here, driven by avoiding failed loops.
- Token *reduction* magnitude depends on the schema. Census tables are **very
  wide** (252 cols), so retrieving even one table is not cheap. A schema with
  **many narrow tables** would show a much larger token-reduction headline.
